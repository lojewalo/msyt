...
impl Control1_9 {
  pub(crate) fn parse(header: &Header, mut reader: &mut Cursor<&[u8]>) -> Result<Self> {
    ...
    let payload_length = header.endianness().read_u16(&mut reader)?;
  ...
  pub(crate) fn write(&self, header: &Header, mut writer: &mut Write) -> Result<()> {
    ...
    header.endianness().write_u16(&mut writer, payload_length as u16)?;
    ...
    for string in &self.strings {
      header.endianness().write_u16(&mut writer, string.field_1)?;
